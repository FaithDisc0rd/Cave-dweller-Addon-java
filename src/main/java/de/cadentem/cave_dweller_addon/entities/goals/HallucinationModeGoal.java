...
public class ApparitionEntity extends Monster implements GeoEntity {
    ...
    public ApparitionEntity(EntityType<? extends ApparitionEntity> entityType, Level level) {
        ...
        // Physics must be ON so the game detects when the ghost's head hits the ceiling
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide()) {
            ...
            // If we collide vertically but are NOT on the ground, we have hit the ceiling.
            if (this.verticalCollision && !this.onGround()) {
                // Stop upward movement so it hangs at the ceiling without jittering
                this.setDeltaMovement(delta.x, 0, delta.z);
            } else {
                // Keep accelerating upwards slowly, capping at a max speed of 0.08
                if (delta.y < 0.08D) {
                    this.setDeltaMovement(delta.add(0, 0.01D, 0));
                }
            }
        }
    }
